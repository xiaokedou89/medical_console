<template>
  <div id="doctors">
    ...
    <el-card class="operate-container">
      ...
      <router-link to="/doctors/addDoctor">
    ...
    <!--设置商品分成弹框-->
    <el-dialog :title="$t('resetBtn')" :visible.sync="proportionFlag" width="40%">
    ...
    <el-dialog :title="$t('agent.index.resetPassword')" :before-close="setPassClosing" :visible.sync="passwordFlag" v-if="passwordFlag" width="40%">
      <el-form ref="passwordForm" :model="setPasswordObj" :rules="rules" size="small" class="sysuserFormRef" label-width="100px">
        <el-form-item :label="$t('agent.index.dialogResetOld')" prop="oldPassword">
          <el-input v-model.trim="setPasswordObj.oldPassword" :placeholder="$t('oldPassPlace')" class="input-width" :clearable="true" show-password />
        </el-form-item>
        <el-form-item :label="$t('agent.index.dialogResetNew')" prop="newPassword">
          <el-input v-model.trim="setPasswordObj.newPassword" :placeholder="$t('newPassPlace')" class="input-width" :clearable="true" show-password />
        </el-form-item>
        <el-form-item :label="$t('medical.addMedical.repeatPassword') + '：'" prop="repeatPassword">
          <el-input v-model.trim="setPasswordObj.repeatPassword" :placeholder="$t('medical.addMedical.repeatPasswordPlace')" class="input-width" :clearable="true" show-password />
        </el-form-item>
        <el-form-item class="reset-btn-group">
      ...
      apiFn(params)
        .then(res => {
          ...
          if (res.code === 20000) {
            ...
            if (this.hospitalFlag || this.departmentFlag || this.removeFlag) {
              this.list = []
            }
            this.flag = false
            this.loading = false
        ...
        if (valid) {
          ...
          api.setDoctorPassword(this.setPasswordObj.id, data)
            ...
            .catch(err => {
              ...
              this.savePassowrdNode.setPasswordFlag = false
            })
        } else {
          this.$message({
            message: this.$t('validFailMes'),
