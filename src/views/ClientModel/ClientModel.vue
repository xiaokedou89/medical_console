<template>
  <div class="canvas_box">
    <!-- 侧边栏 -->
    <div class="canvas_left_aside" v-if="!isMobile" :style="iconFlag === true ? 'margin-left: -0px;' : 'margin-left: -220px;'">
      <!-- 侧边栏编辑颜色 -->
    ...
    <!-- 移动端侧边栏 -->
    <div class="bottom-side" v-else>
      <ul class="bottom-ul">
        <li class="bottom-item" v-for="(item, index) in information.Meshes" :key="index">
          <!-- <div class="item-l" :style="{background: item.Color}" @click="changeColor(item)">
          </div> -->
          <el-color-picker class="item-l" v-model="item.Color" show-alpha @change="changeColor(item)" />
    ...
    <div class="canvas_main">
      <!-- 单个 -->
      <div class="canvas_tag" v-show="showOne" @click="onCanvasTagMouseClick($event)">
      ...
      <!-- <div class="canvas_button">
        <el-button type="primary" size="small" @click="saveInformation">{{ $t('orders.model.save') }}</el-button>
      </div> -->
    </div>
  </div>
</template>
...
import { getAppOrder } from '@/api/Orders/orders'
  ...
  methods: {
      ...
      getAppOrder(this.id)
        .then(res => {
          if (res.code === 20000) {
            ...
            let title = this.order.name === null || this.order.name == '' ? '订单模型' : `订单: ${this.order.name}`
    ...
    getModel() {
      ...
      if (this.order.modelTag) {
        this.information = JSON.parse(JSON.parse(this.order.modelTag).tagInfo)
      } else {
    ...
    loadModel() {
      ...
      console.dir(this.modelDetail)
      // c[0]
      if (this.modelDetail.mapData.length != 0) {
        var textureLoader = new Three.TextureLoader(manager)
        // 腾讯云
        _this.texture = textureLoader.load(this.url + this.modelDetail.mapData[0])
      }
      ...
      // c[0]
      loader.load((this.url + this.modelDetail.modelData), function(obj) {
        _this.object = obj
        _this.loading = false
        _this.setMesh()
      }, onProgress, onError)
    ...
    saveInformation() {
      ...
      const data = {
        tagInfo: JSON.stringify(this.information)
      }
      changeModelTag(this.id, data)
        .then(res => {
          if (res.code === 20000) {
            this.$message({
...
.bottom-side {
  ...
  box-shadow: 0 1px 3px 3px rgb(255, 255, 255);
  ...
  .bottom-ul {
    ...
    .bottom-item {
      ...
      .item-l::v-deep .el-color-picker__trigger {
        width: 80px;
        height: 40px;
      }
      .item-title {
